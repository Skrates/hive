...
5. Treat exit status zero as success only because the Hive CLI waits for the live supervisor to confirm the same owner-only binding revision. On failure, report the safe error and leave routing unclaimed; do not write a binding file manually and do not choose a fallback task.
